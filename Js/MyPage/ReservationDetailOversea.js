$(document).ready(function () {
    ...
    $("#btnGoList").on("click", function () {
        location.href = PATH_MYPAGE + "ReservationList.aspx";
    });
    // 예약취소
    $("#btnResCancel").on("click", function () {
        if ($("#isTicketPass").val() !== "true") {
            ...
            $("#ticketPassRefundInformationReason").show();
        }
    });
    // 티켓/패스 환불 안내 팝업 확인 클릭
    $("#ticketPassRefundReasonInfoConfirm").on("click", function () {
        $("#ticketPassRefundInformationReason").hide();
...
function funcInit() {
    if ($("#hidResDetailErr") != undefined && $("#hidResDetailErr").val() == "Y") {
        funcGenerateCommonPopup("마이페이지", "잘못된 접근입니다.", "MyPageResDetailErr");
    }
}
