...
$(document).ready(function () {
    ...
    $("#btnNPay").on("click", function () {
        ...
        var returnUrl = SERVICE_URL + $("#hidOrganUrl").val() + "AddPaymentOverseaComplete.aspx?key=" + $("#hidKey").val() + "&tempReservationId=" + tempReservationId + "&type=" + paymentType;
...
function funcCalcPrice() {
    ...
    if (totalActPrice > 0) {
        ...
        $("#strNPayPrice").text(Math.max(0, (basicPromotionAmount + accPromotionAmount)).toNumberWithComma() + " 원");
        $("#divNPayPrice").show();
    } else {
        if ($("#hidPromotionYn").val() == "Y") {
            PROMOTION_DATA.forEach(function (promotion) {
