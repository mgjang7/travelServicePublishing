...
function funcCheckReservation() {
    ...
    $.ajax({
        url: PATH_STATIC_AJAX + "AjaxBalancePaymentOverseaProcess.aspx",
        ...
        success: function (res) {
            ...
            if (res.Result == 10000) {
                content = content.replace("#CONTENT#", "고객 님의 잔금결제가 ‘ <strong class='complete'>완료</strong> ’ 되었습니다.");
                content = content.replace("#BTN_ID#", "btnReservationComplete");
            } else if (res.Result == 40 || res.Result == 50 || res.Result == 60 || res.Result == 70) {
        ...
        error: function () {
            content = content.replace("#CONTENT#", "잔금결제 진행 중 오류가 발생하였습니다.<br />다시 시도해주십시오.");
            content = content.replace("#BTN_ID#", "btnErrorBeforeReservation");
        },
        complete: function () {
...
resultTemplate += "    <div class='alt_t'>잔금결제 알림</div>";
